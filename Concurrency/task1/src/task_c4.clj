...
(defn notify-msg
  ...
  (let [bill (state :bill),
        ...
        (if (>= (bill ware) (buffer ware))
          (try
            (do
              (swap! storage-atom (fn [x](- x needed)))     ;retrieve from storage
              (update buffer ware (fn [x] (+ x needed))))   ;put into internal buffer
            (catch IllegalStateException _buffer))          ;handle exception if items could not be retrieved
          buffer)]
    (if (= bill internal-buffer)                            ; if all needed items are retrieved into internal buffer
      (do
        (Thread/sleep (state :duration))                    ;"starting new cycle" with given duration
        (send ((state :target-storage) :worker) supply-msg (state :amount)) ;notifying storage
